...
- **`auxiliary-project-onboarding`** (Initialization phase, runs exactly
...
- **`auxiliary-ticket-intake`** (Initialization phase, conditional on a ticket key
  ...
  stages.
- **`auxiliary-bolt-push-log`** (Construction phase, per unit of work): after a
  ...
  `auxiliary-implementation-log.md` — repo, branch, base branch, one-line summary —
...
From here on, `auxiliary-ticket-intake` operates entirely against whatever
...
`auxiliary-ticket-intake` seeds each new ticket's `codekb/` from a shared,
...
1. Refresh the mirror's repos to their release-branch tips (the same
   fetch/checkout/reset routine `auxiliary-ticket-intake` Step 4 runs
...
- `aidlc/spaces/<space>/knowledge/repo-catalog.md` — the repo/branch/clone-URL
  ...
  once by `auxiliary-project-onboarding` (the first ticket run against a
...
- `aidlc/spaces/<space>/knowledge/architecture-overview.md` and
  `coding-standards.md` — also written once by `auxiliary-project-onboarding`,
